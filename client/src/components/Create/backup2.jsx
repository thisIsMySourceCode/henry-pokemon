...
{
  let input = e.target
  let re = new RegExp("^"+input.getAttribute("pattern")+"$")
    if ( re.test( input.value ) || input.value === '' ) {
      input.classList.remove('error')
      let    id  = input.id,
      output = {}
      input = input.value
      output[id]= (id === 'nombre') ? sanitize(input) : input    
...
console.log(post.nombre)
  ...
  <label>Name
  <input type='text' onInput={(e)=>invalido(e)} id='nombre' name='nombre' pattern="[a-zA-Z].{3,}" minLength='3' autoFocus required='required' placeholder='name is required (Alpha only) ...'/>
  </label>
  </div>
  <div className='column'>

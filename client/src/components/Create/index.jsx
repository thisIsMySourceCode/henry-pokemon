...
import { create, postEdit }        from '../../actions';
...
handleSelector = (e) => {
  ...
  const out = ( isNaN(second) ) 
            ? [{name : type[first]}] 
            : [{name : type[first]},{name : type[second]}]
  let posteo=[]
      posteo['types'] = out
    ...
    <fieldset>
      ...
      <div className='row'>
        <div className='wide_column'>
          ...
          <label>Name
            ...
            placeholder='name is required (Alpha only) ...'/>
          </label>
        </div>
        <div className='column'>
          <label>1st Type</label>
            <select  defaultValue={'normaldefault'} id='first' name='first' 
      ...
      <div className='row'>
        <div className='wide_column'>
          <label>Picture URL
            ...
            required='required' placeholder='url is required: http only'
      ...
      <div className='row'>
        <div className='wide_column'>
          ...
          <input type="range" id="attack" name="attack" min="10" max="100" 
          defaultValue={post.attack} step="10" onChange={(e)=>handleSlider(e)}/>
        </div>
        <div className='column'>
          <label>Hit Points: <span id='hpvalue'>{post.hp}</span></label>
          <input type="range" id="hp" name="hp" min="10" max="1000" 
          defaultValue={post.hp} step="11" onChange={(e)=>handleSlider(e)}/>
        </div>
      </div>
      <div className='row'>
        <div className='wide_column'>
          <label>Defense: <span id='defensevalue'>{post.defense}</span></label>
          <input type="range" id="defense" name="defense" min="10" max="100" 
          defaultValue={post.defense} onChange={(e)=>handleSlider(e)} step="10"/>
        </div>
        <div className='column'>
          <label>Height: <span id='heightvalue'>{post.height} cm</span></label>
          <input type="range" id="height" name="height" min="1" max="200" 
          defaultValue={post.height} step="1" onChange={(e)=>handleSlider(e)}/>
        </div>
      </div>
      <div className='row'>
        <div className='wide_column'>
          <label>Speed: <span id='speedvalue'>{post.speed}</span></label>
          <input type="range" id="speed" name="speed" min="10" max="100" 
          defaultValue={post.speed} step="10" onChange={(e)=>handleSlider(e)}/>
        </div>
        <div className='column'>
          <label>Weight: <span id='weightvalue'>{post.weight} kg</span></label>
          <input type="range" id="weight" name="weight" min="10" max="100" 
          defaultValue={post.weight} step="10" onChange={(e)=>handleSlider(e)}/>
        </div>
      </div>
      <div id='foot'>
        <div id='left'>
          <b>AVISO</b>: Todo form que se precie de tal tiene que contener al menos un disclaimer legal de alg&uacute;n tipo. 
          (c) 2022. Luciano Federico Pereira
        </div>
        <div id='right'>
